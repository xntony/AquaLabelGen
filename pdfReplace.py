...
Replaces the placeholder label image inside an AtemoAqua PDF label template
with a user-supplied "customLabels" image, WITHOUT stretching or distorting
it in the narrow sidebar slots.

-------------------------------------------------------------------------
WHY THE OLD APPROACH BROKE
-------------------------------------------------------------------------
The old script called:

    page.replace_image(xref, stream=image_bytes)

`replace_image()` only swaps the raw image *bytes* behind an existing PDF
image object (xref). It does NOT touch the placement matrix that was
already baked into the page's content stream. That matrix maps the image's
unit square onto whatever rectangle the *template* originally used for
that image -- and in this template, ALL 8 label slots (6 main + 2 side)
point at the SAME xref, just with different placement rectangles.

Because the matrix always maps the image to completely fill the target
rectangle (there is no "fit" concept in a raw `cm ... Do` placement), any
image whose aspect ratio doesn't match the rectangle's aspect ratio gets
squashed/stretched to fill it. That's exactly what happened to the two
narrow vertical side slots (rects ~133pt wide x ~409pt tall) when fed a
landscape (474x266) source image.

Manually pre-rotating the bytes with PIL (as the old script did) fixes the
*orientation* but not the *distortion*, because the rotated image is still
forced to fill a rectangle with a different aspect ratio than the rotated
image itself.

-------------------------------------------------------------------------
NEW APPROACH
-------------------------------------------------------------------------
Instead of fighting `replace_image()`, we treat the template as a layout
of placeholder boxes:

  1. Inspect every image on every page and get its exact placement
     rectangle via `page.get_image_info(xrefs=True)` (no hard-coded
     coordinates -- geometry is read from the PDF itself).
  2. Classify each rectangle as "main" or "side" purely from its own
     geometry (tall-and-narrow => side slot => needs 90 degree rotation).
  3. Cover the original image in that exact rectangle with an opaque
     rectangle, so none of the old (stretched) pixels can show through
     around the edges of the new, correctly-fitted image.
  4. Compute a destination rectangle that preserves the source image's
     aspect ratio (accounting for rotation) and is centered inside the
     original placement rectangle.
  5. Use `page.insert_image()` -- not `replace_image()` -- to draw the
     new image into that destination rectangle. `insert_image()` embeds
     the image as a new XObject and draws it with its own placement
     matrix, so it is completely decoupled from the old, stretched
     placement.
  6. The image is embedded exactly ONCE per page and reused (by xref)
     for every subsequent placement on that page, instead of embedding a
     fresh copy per slot -- keeping file size down.

This never rasterizes the page, never flattens anything, and never
touches any content outside the exact rectangles the original images
occupied, so all other text/vector graphics on the template are left
alone.

NOTE ON THE "COVER" STEP: this script hides the old image by painting a
white rectangle over its exact original bounding box, then drawing the
new image on top. It does not delete the old image object from the PDF's
internal resources (that would require redaction, which risks clipping
any vector art that happens to overlap the box). If your template's
background inside each label box is not white, change COVER_COLOR below.
"""
