...
def main():
    if len(sys.argv) < 3:
        print("Usage: python serverReplace.py <number> <output_filename.pdf>")
        print("  <number> selects the template: AtemoAquaCustomLabels_<number>each.pdf")
        print("  Example: python serverReplace.py 6 output.pdf")
        sys.exit(1)
    ...
    # (no PIL re-encoding / pre-rotation needed).
